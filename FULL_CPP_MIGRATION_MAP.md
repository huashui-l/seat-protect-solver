...
| multi-group rebuild/LNS | conflict LNS and pattern MIPs | native LNS eligibility, frozen infant scoring, matching, geometric options and local set-partition MIP implemented; candidate traversal uses documented lexical order instead of unfrozen Python hash order; search and production integration remain pending | PARTIAL_CPP | master only | ACTIVE |
