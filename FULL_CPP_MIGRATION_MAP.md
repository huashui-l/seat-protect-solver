...
| multi-group rebuild/LNS | conflict LNS and pattern MIPs | native LNS eligibility, frozen infant scoring, matching, geometric options, local set-partition MIP, dynamic candidate ranking, ejection-chain component search, late-history acceptance, rollback and production stage integration implemented; lexical traversal is used for the unfrozen Python candidate set | PARTIAL_CPP | master only | ACTIVE |
...
| elite pattern store | `record_elite_pattern`, `_elite_pattern_eviction_candidate`, stage capture | native identity/resource/conflict handling, replacement, pinning and stable eviction with direct Python replay; construction/repair/VND capture and affected-group scores integrated; construction-frozen conflict-diversity activation and current-state candidate recording implemented; structured/special generation and protected MIP captures integrated; Rich restricted pattern consumer integrated | PARTIAL_CPP | no | ACTIVE |
| restricted/protected master | Python HiGHS stages | native protected component MIP and Rich restricted MIP/local branching functions match frozen Python on complete legal entry contexts; restricted production stage consumes the preserved Rich state and complete elite store with LNS carry; raw Rich no longer invokes the independent RR master | PARTIAL_CPP | yes | ACTIVE, semantics differ |
