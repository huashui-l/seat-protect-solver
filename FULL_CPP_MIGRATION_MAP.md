...
| Rich pricing-cache geometry | `_build_group_pricing_cache` and structured window filtering | native ordered placement domains, shifted coordinates, adjacency and hole metadata match frozen Python including reserved-seat and empty-window boundaries; DFS search and production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich DFS rectangle bounds | `_price_group_exact_dfs` geometric workspace and bound arrays | native coordinate indexes, seat prefix sums, capacity/span and per-depth coupled passenger bounds match frozen Python arrays; branch search and production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich DFS resource/care workspace | `_price_group_exact_dfs` static workspace and caregiver helpers | native multiword resource/SSR masks, signature lookup, caregiver eligibility, reachability and dominance-state encoding match frozen Python; full search, workspace reuse and production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich DFS symmetry | `_price_group_exact_dfs` raw prefilter, physical classes and rank pruning | native raw passenger identity excluding hostnum, physical-domain classes, rounded-cost ranks and partial-state rejection match frozen Python; DFS traversal consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich pricing dynamic costs | baby pairs/relaxation, placement and column reduced costs, flagged-location dual aggregation | native ordered baby pairs, placement costs, first-activation SSR costs, Phase-I and column reduced costs differential passed; DFS and special-pricing consumers pending | PARTIAL_CPP | no | ACTIVE |
