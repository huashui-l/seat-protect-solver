...
| Rich complete placement domain | `_placement_options` | native full seat/protection-mode enumeration, costs and SSR location/flag metadata; frozen Python differential passed; structured generator consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich pattern assembly | `_pattern_from_placements`, `_caregiver_ok` | native placement-to-column assembly, conditional SSR coefficients, infant same-group correction and caregiver adjacency differential passed; structured/master consumers pending | PARTIAL_CPP | no | ACTIVE |
| Structured rigid/relaxed layers | `generate_structured_group_patterns` prefix | native target backtracking, row shifts/mirrors, subrow moves and stable dedup/source replacement match frozen Python; remaining layers and orchestration pending | PARTIAL_CPP | no | ACTIVE |
| Structured group/window ordering | `generate_structured_group_patterns` ordering and row windows | native primary/difficult eligibility, full-resource activation and complete/truncated window order match frozen Python; generator orchestration pending | PARTIAL_CPP | no | ACTIVE |
| Structured value-block layers | `generate_structured_group_patterns` value-block prefix | native ordinary/global blocks, anchor ordering, exact subset matching and stable source replacement differential passed; rebuilt search and production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich pricing-cache geometry | `_build_group_pricing_cache` and structured window filtering | native ordered placement domains, shifted coordinates, adjacency and hole metadata match frozen Python; connected to native DFS; structured production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich DFS rectangle bounds | `_price_group_exact_dfs` geometric workspace and bound arrays | native coordinate indexes, seat prefix sums, capacity/span and coupled passenger bounds match frozen Python; connected to native DFS; structured production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich DFS resource/care workspace | `_price_group_exact_dfs` static workspace and caregiver helpers | native multiword resource/SSR masks, signature lookup, caregiver reachability and dominance-state encoding match frozen Python; connected to native DFS with workspace reuse; structured production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich DFS symmetry | `_price_group_exact_dfs` raw prefilter, physical classes and rank pruning | native raw passenger identity, physical-domain classes and ranks match frozen Python; connected to native DFS; structured production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich pricing dynamic costs | baby pairs/relaxation, placement and column reduced costs, flagged-location dual aggregation | native ordered baby pairs, placement/activation costs, Phase-I and column reduced costs match frozen Python; connected to native DFS; structured/special production consumers pending | PARTIAL_CPP | no | ACTIVE |
| Rich rebuilt pricing DFS | `_price_group_exact_dfs` | native full-domain traversal, constraints, stable ordering, geometry/resource/care/symmetry pruning, dominance, historical starts, negative pool, workspace reuse and termination diagnostics match direct frozen-function replay; structured production orchestration pending | PARTIAL_CPP | no | ACTIVE |
| Structured generation orchestration | `generate_structured_group_patterns` | native complete rigid/relaxed/value/global/rebuilt sequence, window deadlines, stable source replacement, affected-group scoring and synchronous recorder calls match complete frozen-function replay; production scheduler/elite callback connection pending | PARTIAL_CPP | no | ACTIVE |
