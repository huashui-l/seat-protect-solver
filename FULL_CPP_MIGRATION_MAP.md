...
| Rich pricing-cache geometry | `_build_group_pricing_cache` and structured window filtering | native ordered placement domains, shifted coordinates, adjacency and hole metadata match frozen Python including reserved-seat and empty-window boundaries; DFS workspace/search and production consumer pending | PARTIAL_CPP | no | ACTIVE |
| Rich DFS rectangle bounds | `_price_group_exact_dfs` geometric workspace and bound arrays | native coordinate indexes, seat prefix sums, capacity/span and per-depth coupled passenger bounds match frozen Python arrays; resource/flag workspace, branch search and production consumer pending | PARTIAL_CPP | no | ACTIVE |
